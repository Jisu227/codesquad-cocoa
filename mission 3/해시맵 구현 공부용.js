...
function hashMap(size) { // 특정 사이즈의 빈 key와 value Array를 만든다.
  this.size = size;
...
// inintArray 메서드 : size를 인수로 받아 해당 size의 빈 배열을 만든다. (초기화 해주는 역할, 내부적으로 실행됨)
...
hashMap.prototype.hash = function (key) {
  ...
  if(isNaN(intKey)) { // 바꿔준 값이 숫자(int형)가 아닐 경우 
    let sumAsciiNum = 0;
...
hashMap.prototype.put = function (key, value) { // put 함수 구현
  if(this.limit >= this.size) throw 'hash table is full';

  let index = this.hash(key);

  // 선형 탐사 -> 선형 탐사를 하는 이유 : 충돌(중복)을 막기 위한 방법 
  while (this.keys[index] !== null) { // 넣고자 하는 index에 이미 값이 있다면
    if(index === this.size -1) { // 근데 그 index가 마지막 인덱스라면! 다음 인덱스가 없어 보내지 못하기 때문에
...
hashMap.prototype.remove = function(key) { // remove 함수 구현 
  let index = this.hash(key);

  // while 문을 통하여 배열을 순회하여 key 값이 일치하지 않을 경우 while문 안을 반복한다.
  while(this.keys[index] !== key) { // keys[index]에 들어있는 key 값과 key가 일치하지 않는다면 
    if(index === this.size -1) { // index가 마지막 index라면 index에 0을 넣어 준다.
      index = 0;
    } else {
      index++;
    }
...
hashMap.prototype.get = function(key) { // 해당 키와 매치는 되는값을 찾아서 리턴하는 역할.
  let index = this.hash(key);
...
hashMap.prototype.isEmpty = function() {
  for(let i = 0; i < this.size; i++) {
    if(this.keys[i] !== null) { // for문을 돌때 keys 배열에 null값이 아닌값이 있다면
...
hashMap.prototype.returnKeys = function() { // 전체 키 목록을 [String] 배열로 리턴한다.
  return this.keys;
}

hashMap.prototype.replace = function(key, value) { // 키-값 으로 기존 값을 대체한다.
  let index = this.hash(key);
...
const newhashMap = new hashMap(4); // 인스턴스 생성 
